...
uint8_t       current_os, mod_primary_mask, fade_interval, num_extra_flashes_off = 0;
Color         underglow, flash_color, saved_color, hsv_none                      = {0, 0, 0};
flashState    flash_state = no_flash;
fadeState     fade_state  = add_fade;
activityState state       = boot;
bool          aesthetic = false, shifty = false;

float song_ussr[][2] = SONG(USSR_ANTHEM);

void set_color(Color new, bool update) { rgblight_sethsv_eeprom_helper(new.h, new.s, new.v, update); }

void save_color(Color to_save) { saved_color = to_save; }

void reset_color(void) { underglow = saved_color; }
...
void check_state(void) {
    ...
    switch (state) {
        case active:
            ...
            if (timer_elapsed(active_timer) < INACTIVE_DELAY) {
                return;
            }
            active_timer = timer_read();
            state        = inactive;
...
void fade_rgb(void) {
    ...
    if (timer_elapsed(fade_timer) < fade_interval) {
        return;
    }
    switch (fade_state) {
        case add_fade:
...
void matrix_scan_user(void) {
    ...
    LEADER_DICTIONARY() {
        ...
        SEQ_ONE_KEY(KC_B) { surround_type(2, KC_8, true); }
        SEQ_ONE_KEY(KC_I) { surround_type(2, KC_MINS, true); }
        SEQ_ONE_KEY(KC_U) { surround_type(4, KC_MINS, true); }
        SEQ_ONE_KEY(KC_S) { surround_type(4, KC_GRAVE, true); }
        SEQ_ONE_KEY(KC_C) { send_unicode_hex_string("00E7"); }
        ...
        SEQ_TWO_KEYS(KC_C, KC_C) { surround_type(2, KC_GRAVE, false); }
        SEQ_THREE_KEYS(KC_C, KC_C, KC_C) { surround_type(6, KC_GRAVE, false); }
        SEQ_ONE_KEY(KC_E) { send_unicode_hex_string("00E8"); }
        SEQ_TWO_KEYS(KC_E, KC_E) { send_unicode_hex_string("00E9"); }
        ...
        SEQ_TWO_KEYS(KC_D, KC_D) { SEND_STRING(".\\Administrator"); }
        ...
        SEQ_THREE_KEYS(KC_M, KC_A, KC_P) { SEND_STRING("https://github.com/qmk/qmk_firmware/tree/master/users/arkag"); }
